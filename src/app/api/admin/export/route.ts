...
export async function GET(request: NextRequest) {
  try {
    ...
    // Professional CSV Export Template - Income Statement Style
    ...
    // Combine title, headers, and data with professional formatting
    const csvRows = [
      ...reportTitle.map(row => row.join(",")),
      ["", ...headers].map(h => `"${h}"`).join(","), // Indent data columns
      ...rows.map(row => ["", ...row].join(",")), // Indent data rows
      "",
      ["", "Total Bookings", "", "", "", "", "", "", "", "", `"${totalGuests}"`, `"₱${totalAmount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}"`, "", "", "", ""].join(","),
      "",
      "",
      ["", `"© ${new Date().getFullYear()} Manuel Resort"`, "", "", "", "", "", "", "", "", "", "", "", "", "", `"Generated by Manuel Resort Booking System"`].join(","),
    ];
...
async function exportSummaryReport(where: any) {
  ...
  // Professional Summary Report CSV - Income Statement Style
  ...
  const summaryRows = [
    ...
    ["", "Total Revenue", "", "", `₱${totalRev.toLocaleString('en-US', {minimumFractionDigits: 2})}`],
    [""],
    ["", "Revenue by Status", "", "", ""],
    ...
    ...bookingsByStatus.map((stat) => [
      ...
      "",
      Number(stat._sum.totalAmount || 0).toLocaleString('en-US', {minimumFractionDigits: 2}),
    ]),
    ["", "Total Revenue", "", "", `₱${totalRev.toLocaleString('en-US', {minimumFractionDigits: 2})}`],
    [""],
    ["", "Revenue by Facility", "", "", ""],
    ...
    [""],
    [""],
    ["", `© ${new Date().getFullYear()} Manuel Resort`, "", "", "Generated by Manuel Resort Booking System"],
  ];
